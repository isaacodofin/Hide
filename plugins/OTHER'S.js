...
const DB_PATH = path.join(__dirname, '../data/messageStore.db/ANTIDELETE.MS/antidelete_mStore.db');
...
function loadAllMessagesFromDB() {
    ...
    try {
        ...
        console.log(`✅ Loaded ${rows.length} messages from DB to memory`);
...
// Store incoming messages (hybrid: RAM + DB)
...
async function storeMessage(sock, message) {
    ...
    try {
        ...
        // 🚀 HYBRID STORAGE: Save to both RAM and DB
...
// Handle message deletion (checks RAM first, then DB)
...
async function handleMessageRevocation(sock, revocationMessage) {
    ...
    try {
        ...
        const deletionChatId = revocationMessage.key.remoteJid;

        const ownerNumber = sock.user.id.split(':')[0] + '@s.whatsapp.net';

        if (deletedBy.includes(sock.user.id) || deletedBy === ownerNumber) return;

        // 🚀 HYBRID RETRIEVAL: Try RAM first, fallback to DB
        ...
        // ✅ FIXED: Determine where to send based on mode

        const sendTargets = [];

        if (mode === 'dm') {

            // Mode: dm - Send to owner DM only
            ...
            // Mode: inbox - Repost in same chat (PM only)
            ...
            // Mode: group - Repost in same chat (groups only)
            ...
            // Mode: all - Send to owner DM + repost in original chat
        ...
        // 🚀 HYBRID CLEANUP: Remove from both RAM and DB
...
Users with Warnings: ${warningCount}`;
    ...
    description: 'Configure antidelete and anti-view once feature',

    usage: '.antidelete [off/dm/inbox/group/all]',
    ...
    execute: async (sock, message, args, context) => {
        ...
        const currentMode = getSetting('antidelete', 'off');
        ...
        updateSetting('antidelete', newMode);

        const modeMessages = {

            'off': '❌ Antidelete disabled',

            'dm': '📨 Antidelete enabled - Sending to owner DM',

            'inbox': '📥 Antidelete enabled - Reposting in PM inbox',

            'group': '👥 Antidelete enabled - Reposting in groups',

            'all': '🌐 Antidelete enabled - Reposting everywhere'
...
export { storeMessage, handleMessageRevocation };ocation };
